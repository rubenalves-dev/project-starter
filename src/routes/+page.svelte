...
<div class="grid grid-cols-2 gap-4">
	...
	<pre class="border p-4 text-xs">
:root &lbrace;
    --white: #ffffff;
    --black: #000000;
    {#each colors as color, i (i)}{@const colorValues = [
				50, 100, 200, 300, 400, 500, 600, 700, 800, 900
			]}{#each colorValues as cv, j (j)}
				--{toSlug(color.name)}-{cv}: {color?.values?.[j] ?? 'transparent'};<span
					style:background={color?.values?.[j]}
					class="inline-block aspect-square w-4"></span>{/each}{/each}
&rbrace;
</pre>
</div>
